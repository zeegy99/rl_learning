...
def visualize_training(maze_obj, q_table, episodes_to_show=1, delay=0.1):
    ...
    while running and episode < episodes_to_show:
        ...
        state = maze_obj.reset()
        done = False
        step_count = 0
        current_epsilon = max(MIN_EPSILON, EPSILON * (EPSILON_DECAY_RATE ** episode))
        
        print(f"Episode {episode + 1} | Epsilon: {current_epsilon:.4f}")
        ...
        while not done and step_count < 50:  # Limit steps to prevent infinite loops
            ...
            # Take action
            new_state, reward, done = maze_obj.step(action)
            ...
            # Draw everything
            draw_maze(screen, maze_obj, maze_obj.current_position, font)
            ...
            info_text = [
                ...
                f"Q-value: {old_q_value:.2f} -> {new_q_value:.2f}"
            ]
...
def train_with_full_visualization():
    ...
    print("Starting visualization of first 10 episodes...")
    print("Press SPACE to pause, close window to stop")
    ...
    for episode in range(0, EPISODES):  # Start from episode 10
        ...
        if episode % 100 == 0:
            visualize_training(new_maze, Q_TABLE, episodes_to_show=1, delay=0.1)
            print(f"Episode: {episode} | Epsilon: {current_epsilon:.4f}")
